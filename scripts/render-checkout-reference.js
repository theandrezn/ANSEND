...
async function render(viewport, output) {
  ...
  await page.evaluate(({ cover, pix }) => {
    document.body.innerHTML = window.AnsendCheckout.renderCheckout({
      ...
      recommendation: { id: "3", title: "Horizonte Azul", producer: "Beatmaker ANSEND", description: "Licença Premium", price: "R$ 149,90", originalPrice: "R$ 199,90", cover, sponsored: true },
...
(async () => {
  const viewports = [
    [1440, 1000], [1280, 960], [1024, 900], [768, 1024], [430, 932], [390, 844], [360, 800],
  ];
  const results = {};
  ...
  if (desktop.shellWidth > 1480.5) throw new Error("Desktop shell exceeds 1480px");
  if (desktop.formWidth < 400 || mobile.formWidth < 320) throw new Error(`Checkout form is compressed: ${JSON.stringify({ desktop, mobile })}`);
