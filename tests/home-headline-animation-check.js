...
async function snapshot(page) {
  return page.evaluate(() => {
    ...
    return {
      ...
      index: title?.dataset.headlineIndex || "",
...
async function run() {
  ...
  try {
    ...
    const hiddenLater = await snapshot(page);
    if (hidden.index !== hiddenLater.index) throw new Error("Headline advanced while hidden");
    await page.evaluate(() => {
    ...
    const reducedPage = await browser.newPage({ viewport: { width: 390, height: 844 }, reducedMotion: "reduce", isMobile: true });
