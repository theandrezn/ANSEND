...
const checks = [
  ...
  ["public catalog query", /function loadPublicPlatformData[\s\S]*?\.eq\("status",\s*"published"\)/],
  ["owned catalog query", /function loadOwnedCatalogItems[\s\S]*?\.eq\("user_id",\s*appState\.authUser\.id\)/],
  ["public profile query", /function loadPublicPlatformData[\s\S]*?\.from\("public_profiles"\)/],
