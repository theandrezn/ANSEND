...
for (const marker of [
  ...
  'html.checkout-standalone-active body[data-route="checkout"] > .page',
  "margin: 0 !important",
...
for (const marker of [
  ...
  "function syncCheckoutStandaloneRoute",
  "mountTarget: appView",
...
for (const marker of [
  "checkout-standalone-boot",
  "checkout-standalone-active",
  "node.remove()",
]) assert(script.includes(marker), `Checkout standalone separation missing marker: ${marker}`);
...
assert(index.includes("checkout-standalone-boot"), "index.html must prevent checkout sidebar flash before JS route hydration");
